...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const API_BASE =
    process.env.VITE_API_URL || process.env.API_URL || "http://localhost:5000";

  try {
    const apiResponse = await fetch(`${API_BASE}/api/stories/${idStr}`);
    if (!apiResponse.ok)
      throw new Error(`Backend returned status ${apiResponse.status}`);
    ...
    const description = escapeHtml(
      story?.content
        ...
        : "Ignite your imagination with powerful AI-driven storytelling tools.",
    );
    const image = escapeHtml(
      story?.coverImage || "https://storysparkai.vercel.app/og-image.jpg",
    );
