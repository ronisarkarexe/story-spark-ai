...
export const useSpeechSynthesis = (
  ...
  const resolveBrowserVoice = useCallback(
    (voiceId: string): SpeechSynthesisVoice | undefined => {
      return browserVoices.find((voice) => getVoiceId(voice) === voiceId);
    },
    [browserVoices],
  );
  ...
  const speakText = useCallback(
    (nextText?: string) => {
      ...
      utterance.onerror = (e) => {
        // Ignore boundary interruptions/stops
    ...
    [isSupported, rateState, pitchState, volumeState, selectedLanguage, selectedVoiceId, resolveBrowserVoice, stop],
  );
  ...
  return {
    ...
    selectedVoiceIndex: browserVoices.findIndex((v) => getVoiceId(v) === selectedVoiceId),
