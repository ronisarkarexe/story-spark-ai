...
export const useSpeechSynthesis = (
  ...
  const languageOptions = useMemo<LanguageOption[]>(() => {
    ...
    return Array.from(counts.entries())
      .map(([lang, voiceCount]) => ({ lang, label: getLanguageLabel(lang), voiceCount }))
  ...
  const speakText = useCallback(
    (nextText?: string) => {
      ...
      utterance.onboundary = (event) => {
        ...
        const nextWordIndex = getWordIndexAtCharIndex(event.charIndex, wordRangesRef.current);
    ...
    [isSupported, pitchState, rateState, resolveBrowserVoice, selectedLanguage, selectedVoiceId, stop, volumeState],
  );
  ...
  useEffect(() => {
    ...
    if (!voiceStillExists) setSelectedVoiceId(voices[0].id);
  ...
  useEffect(() => {
    ...
    if (!languageStillExists) setSelectedLanguage(languageOptions[0].lang);
  }, [languageOptions, selectedLanguage]);

  const progress = useMemo<SpeechProgress>(() => {
    const totalWords = wordRangesRef.current.length;
    if (totalWords === 0) return { currentWordIndex: 0, totalWords: 0, percentage: 0 };
    const boundedCurrentWordIndex = Math.min(Math.max(currentWordIndex, 0), totalWords - 1);
    return {
