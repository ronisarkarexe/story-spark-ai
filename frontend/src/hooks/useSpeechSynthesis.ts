...
export const useSpeechSynthesis = (
  ...
  const speakText = useCallback(
    (nextText?: string) => {
      ...
      utterance.onend = () => {
        setIsSpeaking(false);
        setIsPaused(false);
        if (wordRangesRef.current.length > 0) {
          setCurrentWordIndex(wordRangesRef.current.length - 1);
        } else {
          setCurrentWordIndex(0);
        }
      };
  ...
  // Stop when text changes while speaking
  useEffect(() => {
  ...
  // Update selected voice when voices change
  useEffect(() => {
  ...
  // Update selected language when language options change
  useEffect(() => {
  ...
  return {
    ...
    setSelectedVoice: (index: number) => {
      if (voices[index]) {
        setSelectedVoiceId(voices[index].id);
      }
    },
