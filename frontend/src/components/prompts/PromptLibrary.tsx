...
import { storyPrompts, searchPrompts } from "../../utils/promptLibrary";

interface Props {
  onInsertPrompt?: (prompt: string) => void;
}

export default function PromptLibrary({ onInsertPrompt }: Props) {
  const [search, setSearch] = useState("");
  const [favorites, setFavorites] = useState<number[]>([]);

  const filtered = useMemo(() => searchPrompts(search), [search]);
        ...
        {filtered.map((prompt) => (
          ...
          >
            ...
            <div className="mt-4 flex gap-3">
              ...
              >
                {favorites.includes(prompt.id) ? "★ Saved" : "☆ Save"}
              </button>
            </div>
          </div>
        ))}
