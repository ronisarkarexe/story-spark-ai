...
const sampleReviews: Review[] = [
  {
    ...
    role: "Fiction Writer",
    feedback:
      "StorySparkAI completely cured my writer's block. The character suggestions are incredibly deep and nuanced.",
    imgSrc: "https://i.pravatar.cc/150?u=sarah",
    rating: 5,
    status: "approved",
  },
  {
    ...
    role: "Screenwriter",
    feedback:
      "I use this tool daily to structure my screenplays. The pacing recommendations are spot on.",
    imgSrc: "https://i.pravatar.cc/150?u=marcus",
    rating: 5,
    status: "approved",
  },
  {
    ...
    role: "Indie Author",
    feedback:
      "Publishing my first novel wouldn't have been possible without the world-building assistance.",
    imgSrc: "https://i.pravatar.cc/150?u=elena",
    rating: 5,
    status: "approved",
  },
  {
    ...
    role: "Blogger",
    feedback:
      "Helps me generate fresh ideas for my weekly posts. It's like having a co-writer available 24/7.",
    imgSrc: "https://i.pravatar.cc/150?u=david",
    rating: 5,
    status: "approved",
  },
  {
    ...
    role: "Student Writer",
    feedback:
      "Great for expanding my vocabulary and learning how to vary my sentence structures.",
  ...
  },
  {
    ...
    feedback:
      "The absolute best tool for drafting quick video scripts and narratives.",
    imgSrc: "https://i.pravatar.cc/150?u=james",
    rating: 5,
    status: "approved",
  },
  {
    ...
    role: "Poet",
    feedback:
      "Surprisingly adept at maintaining rhythm and suggesting evocative imagery.",
    imgSrc: "https://i.pravatar.cc/150?u=chloe",
    rating: 5,
    status: "approved",
  },
  {
    ...
    role: "Storyteller",
    feedback:
      "I use it to build D&D campaigns. The lore generation is simply unmatched.",
    imgSrc: "https://i.pravatar.cc/150?u=robert",
    rating: 5,
    status: "approved",
  },
];
...
const WriterFeedbackComponent = () => {
  ...
  const displayData =
    apiData.length > 4 ? apiData : [...apiData, ...sampleReviews].slice(0, 10);
  ...
  const renderMarqueeRow = (items: Review[], direction: "left" | "right") => {
    ...
    return (
      <div className="hidden md:flex w-max gap-4 sm:gap-6 pause-on-hover py-2">
        <div
          className={`flex w-max gap-4 sm:gap-6 items-stretch ${animationClass}`}
        >
          {[...items, ...items, ...items, ...items].map((writer, i) => (
            <ReviewCard
              key={`${writer._id ?? writer.name}-${direction}-${i}`}
              writer={writer}
            />
          ))}
        </div>
  ...
  if (isLoading)
    return (
      <div className="py-12 text-center text-slate-500 dark:text-slate-400 text-sm font-medium animate-pulse">
        Loading reviews...
      </div>
    );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          ...
          <p className="mt-4 text-base sm:text-lg text-slate-600 dark:text-slate-400 max-w-xl mx-auto leading-relaxed">
            Join thousands of creators who are elevating their storytelling and
            building worlds with Story Spark AI.
          </p>
        ...
        <div className="relative mb-12 sm:mb-16 marquee-mask -mx-4 sm:-mx-6 lg:-mx-8 px-4 sm:px-6 lg:px-8">
          <div
            className="md:hidden flex overflow-x-auto snap-x snap-mandatory gap-4 pb-6 pt-2"
            style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}
          >
            {displayData.map((writer: Review, i: number) => (
              <div
                key={`mobile-${writer._id ?? writer.name}-${i}`}
                className="snap-center shrink-0 items-stretch flex"
              >
