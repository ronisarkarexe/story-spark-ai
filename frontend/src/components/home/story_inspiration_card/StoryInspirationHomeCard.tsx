...
const StoryInspirationHomeCard = () => {
  const featured = FEATURED_IDS.map((id) =>
    inspirationData.find((s) => s.id === id),
  ...
  return (
    <section className="rounded-xl border border-indigo-100 dark:border-indigo-500/20 bg-white dark:bg-slate-900/50 shadow-sm overflow-hidden">
      ...
      <div className="p-4 space-y-3">
        {featured.map((story) => {
          const genreColor =
            FEATURED_GENRE_COLORS[story.genre] ?? "bg-slate-100 text-slate-700";
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <span
                  className={`inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded-full text-[10px] font-semibold ${genreColor}`}
                >
      ...
      <div className="px-4 pb-4">
        <div className="flex flex-wrap gap-1.5 mb-4">
          {genres.map((genre) => {
            const color =
              FEATURED_GENRE_COLORS[genre] ?? "bg-slate-100 text-slate-600";
