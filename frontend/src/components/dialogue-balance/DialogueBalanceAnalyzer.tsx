...
import { analyzeDialogueBalance } from "../../utils/dialogueBalanceAnalyzer";
...
export default function DialogueBalanceAnalyzer({ story, onRefresh }: Props) {
  const report = useMemo(() => analyzeDialogueBalance(story), [story]);
    ...
    <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 mb-6">
        <div className="rounded-lg border border-zinc-700 p-5">
          <h3 className="text-lg font-semibold text-white">Dialogue</h3>
      ...
      <div className="rounded-lg border border-zinc-700 p-4">
        ...
        <p className="mt-2 text-gray-300">{report.suggestion}</p>
      </div>
    </div>
  );
}
