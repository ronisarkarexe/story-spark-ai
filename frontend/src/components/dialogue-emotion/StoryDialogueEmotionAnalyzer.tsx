...
import { analyzeDialogueEmotion } from "../../utils/storyDialogueEmotionAnalyzer";
...
export default function StoryDialogueEmotionAnalyzer({
  ...
  const report = useMemo(() => analyzeDialogueEmotion(story), [story]);
      ...
      <div className="mb-8">
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {report.summaries.map((summary) => (
            ...
            >
              <h4 className="font-semibold text-white">{summary.character}</h4>
        ...
        {report.dialogues.map((item) => (
          <div key={item.id} className="rounded-lg border border-zinc-700 p-5">
            <div className="flex items-center justify-between">
              <h3 className="font-semibold text-white">{item.character}</h3>
            ...
            <p className="mt-3 italic text-gray-300">"{item.dialogue}"</p>

            <p className="mt-3 text-sm text-gray-400">
              Confidence: {item.confidence}%
            </p>

            <p className="mt-2 text-indigo-400">{item.suggestion}</p>
          </div>
        ))}
      </div>
    </div>
