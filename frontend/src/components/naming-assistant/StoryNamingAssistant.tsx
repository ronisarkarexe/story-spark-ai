...
import { EntityType, generateNames } from "../../utils/storyNamingAssistant";

interface Props {
  onInsert?: (name: string) => void;
}

export default function StoryNamingAssistant({ onInsert }: Props) {
  const [genre, setGenre] = useState("Fantasy");

  const [entity, setEntity] = useState<EntityType>("Character");

  const names = useMemo(() => generateNames(genre, entity), [genre, entity]);
        ...
        {names.map((item) => (
          ...
          >
            <div>
              ...
              <p className="text-sm text-gray-400">{item.entityType}</p>
            </div>
