...
import { generateRewriteSuggestions } from "../../utils/paragraphRewriteSuggestions";
...
export default function ParagraphRewriteSuggestions({
  ...
  const suggestions = useMemo(
    () => generateRewriteSuggestions(paragraphs[selected] || "", story),
    [selected, story],
  );
      ...
      <div className="space-y-5">
        {suggestions.map((item) => (
          <div key={item.id} className="rounded-lg border border-zinc-700 p-5">
            ...
            <button
              onClick={() => onReplace(item.content)}
