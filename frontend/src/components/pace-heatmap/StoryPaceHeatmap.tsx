...
export default function StoryPaceHeatmap({ story, onRefresh }: Props) {
  const sections = useMemo(() => analyzeStoryPace(story), [story]);
        ...
        {sections.map((section) => (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-sm text-gray-300">{section.pace}</span>
            </div>
            ...
            <p className="mt-3 text-gray-300">{section.suggestion}</p>
          </div>
        ))}
      </div>
    </div>
