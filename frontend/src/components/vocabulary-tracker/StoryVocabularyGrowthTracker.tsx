...
export default function StoryVocabularyGrowthTracker({
  ...
  const report = useMemo(() => analyzeVocabulary(story), [story]);
  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-3 mb-6">
        <StatCard title="Total Words" value={report.totalWords} />

        <StatCard title="Unique Words" value={report.uniqueWords} />

        <StatCard title="Diversity" value={`${report.diversityScore}%`} />
      </div>
          ...
          {report.overusedWords.map((item) => (
            ...
            >
              <div className="flex justify-between">
                ...
                <span className="text-indigo-400">{item.count} uses</span>
              </div>
...
function StatCard({ title, value }: { title: string; value: string | number }) {
  return (
    <div className="rounded-lg border border-zinc-700 p-4 text-center">
      ...
      <p className="mt-2 text-2xl font-bold text-indigo-400">{value}</p>
    </div>
  );
}
