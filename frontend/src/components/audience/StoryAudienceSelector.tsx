...
import { audienceOptions, AudienceOption } from "../../utils/storyAudience";

interface Props {
  prompt: string;
}

export default function StoryAudienceSelector({ prompt }: Props) {
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {audienceOptions.map((audience) => (
          ...
          >
            ...
            <p className="text-sm mt-2">{audience.description}</p>
          </button>
        ))}
      </div>
      ...
      <button className="mt-6 px-5 py-2 bg-indigo-600 rounded-lg text-white">
        Generate Story
      </button>
    </div>
  );
