    ...
    <header className="sticky top-0 z-50 w-full border-b border-slate-200/70 bg-white/90 backdrop-blur-md dark:border-white/10 dark:bg-[#0B1120]/80">
      ...
      {menuOpen && (
        <div className="space-y-1 border-t border-slate-200/70 px-4 py-3 lg:hidden dark:border-white/10">
          <NavLink to="/" end className={linkClass} onClick={handleNavClick}>Home</NavLink>
          <NavLink to="/explore" className={linkClass} onClick={handleNavClick}>Explore</NavLink>
          <NavLink to="/stories" className={linkClass} onClick={handleNavClick}>Stories</NavLink>
          <NavLink to="/chat" className={linkClass} onClick={handleNavClick}>AI Chat</NavLink>
          {loggedIn && (
            <NavLink to="/dashboard" className={linkClass} onClick={handleNavClick}>Dashboard</NavLink>
          )}
          {loggedIn ? (
            <button
              onClick={handleLogout}
            ...
            <Link to="/login" className={linkClass} onClick={handleNavClick}>Login</Link>
          )}
        </div>
      )}
    </header>
