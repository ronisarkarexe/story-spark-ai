...
const NavListComponent: React.FC = () => {
  ...
  const getMobileLinkClass = (isActive: boolean) =>
    `flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-semibold transition-all duration-300 border ${
      isActive
  ...
  const renderMobileNavContent = (label: string, isActive: boolean) => (
    <>
      <span className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${isActive ? "bg-blue-500 scale-100 shadow-[0_0_8px_#3b82f6]" : "bg-transparent scale-0"}`} />
      {label}
    </>
  );
          ...
          <div className="xl:hidden mt-2 px-1 pb-4 flex flex-col gap-1.5 border-t border-slate-200/70 dark:border-white/10 pt-3">
            ...
            {isLogin ? (
              <>
                ...
                <button
                  onClick={() => { setMenuOpen(false); handelLogout(); }}
                  className="text-left text-slate-600 dark:text-slate-400 py-3 px-4 font-semibold hover:bg-slate-100 dark:hover:bg-white/5 rounded-xl transition-all"
                >
                  LOGOUT
                </button>
              </>
