...
const FeatureCard = ({ feature }: { feature: Feature }) => {
  ...
  return (
    <div style={{ perspective: "1000px" }} className="h-full w-full">
      <div ref={cardRef} className={`relative overflow-hidden backdrop-blur-xl border border-slate-200/50 dark:border-white/10 rounded-3xl p-6 sm:p-8 transition-shadow duration-500 shadow-sm group cursor-pointer ${feature.bgClass} hover:shadow-[0_0_40px_rgba(255,255,255,0.12)] h-full w-full`}>
        ...
        <div ref={contentRef} className="relative z-10 pointer-events-none">
          ...
          <h3 className="text-lg sm:text-xl font-bold text-white mb-2.5 sm:mb-3 tracking-tight group-hover:text-blue-100 transition-colors duration-300">{feature.title}</h3>
...
const HeroParticles = () => {
  ...
  useGSAP(() => {
    ...
    particles.forEach((particle, i) => {
      ...
      gsap.to(particle, { x: config.xMove, y: config.yMove, scale: 1.4, opacity: 0.9, duration: config.dur / 2, ease: "sine.inOut", yoyo: true, repeat: -1, delay: i * 0.3 });
    });
  }, { scope: containerRef });
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 pointer-events-none overflow-hidden select-none" style={{ zIndex: 1 }} aria-hidden="true">
      {PARTICLE_CONFIG.map((p, i) => (
        <span key={i} className="gsap-particle" style={{ position: "absolute", borderRadius: "9999px", left: p.left, top: p.top, width: p.size, height: p.size, opacity: 0.3, background: `radial-gradient(circle, ${p.color}, ${p.color}88, transparent)`, boxShadow: `0 0 ${p.size * 4}px ${p.color}` }} />
      ))}
    </div>
  );
};
...
const HeroSectionComponent = () => {
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="relative min-h-screen bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100 overflow-hidden transition-colors duration-300 w-full">
      <div className="absolute top-[-10%] left-[-10%] w-[500px] h-[500px] bg-sky-200/40 dark:bg-blue-600/10 rounded-full blur-[120px] pointer-events-none -z-10 select-none" />
      <div className="absolute top-[20%] right-[-10%] w-[500px] h-[500px] bg-fuchsia-200/30 dark:bg-purple-600/10 rounded-full blur-[120px] pointer-events-none -z-10 select-none" />
      <HeroParticles />

      <div className="relative overflow-hidden w-full" onMouseMove={handleMouseMove}>
        <motion.div variants={itemVariants} className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-16 sm:pt-20 sm:pb-20 text-center w-full">
          <div ref={badgeRef} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/80 dark:bg-slate-900/60 border border-slate-200 dark:border-white/10 backdrop-blur-md mb-8 shadow-sm cursor-pointer select-none">
          ...
          <motion.h1 variants={itemVariants} className="text-3xl sm:text-5xl lg:text-7xl font-extrabold tracking-tight mb-6 sm:mb-8 leading-tight select-none">
            ...
            <span className="hero-gradient-text pb-2">
              <Typewriter phrases={["AI-Driven Storytelling", "Creative Story Generation", "Smart Writing Assistant"]} />
            </span>
          </motion.h1>
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4 select-none">
            <button
              onClick={() => { setIsNavigating(true); setTimeout(() => { window.location.href = "/stories"; }, 400); }}
              disabled={isNavigating}
              className={`w-full sm:w-auto px-6 sm:px-8 py-3 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-xs sm:text-sm font-bold shadow-md transition-all duration-150 flex items-center justify-center gap-2.5 uppercase tracking-wider ${isNavigating ? "opacity-75 cursor-not-allowed" : "hover:from-blue-500 hover:to-indigo-500 hover:scale-[1.02] active:scale-[0.98] cursor-pointer"}`}
            >
              {isNavigating ? <><svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v4l3-3-3-3v4a8 8 0 00-8 8h4z" /></svg><span>Loading...</span></> : <><i className="fa fa-wand-magic-sparkles text-sm"></i><span>Get Started</span></>}
            </button>
            <Link to="/collab" className="w-full sm:w-auto">
              <button className="w-full sm:w-auto px-6 sm:px-8 py-3 rounded-xl bg-white/80 dark:bg-[#111827]/40 backdrop-blur-md border border-slate-200 dark:border-white/10 text-slate-700 dark:text-white text-xs sm:text-sm font-bold shadow-sm hover:bg-slate-50 dark:hover:bg-[#111827]/80 hover:scale-[1.02] active:scale-[0.98] transition-all duration-150 flex items-center justify-center gap-2.5 cursor-pointer uppercase tracking-wider">
                <span>✍️</span><span>Collab Mode</span>
              </button>
            </Link>
          </div>
          ...
          <div className="absolute inset-0 -z-10 pointer-events-none overflow-hidden select-none">
            <div className="hero-cursor-stars absolute inset-0" aria-hidden="true">
              {stars.map((star) => (
                <span key={star.id} className={`hero-cursor-star ${star.size > 12 ? "hero-cursor-star-large" : ""}`} style={{ left: star.x, top: star.y, width: star.size, height: star.size }} />
              ))}
            </div>
          </div>
        </motion.div>

        <motion.div variants={itemVariants} className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20 sm:pb-28 w-full">
