...
const features = [
  ...
  {
    title: "AI Co-Writer",
    description: "Let our advanced AI models suggest the next perfect sentence to keep your momentum going.",
  ...
  },
  {
    ...
    description: "Publish your stories, gather likes, and interact with other creators.",
...
const FeatureCard = ({ feature }: { feature: Feature }) => {
  ...
  return (
    <div style={{ perspective: "1000px" }} className="h-full w-full box-border">
      <div
        ref={cardRef}
        className={`relative overflow-hidden backdrop-blur-xl border border-slate-200/50 dark:border-white/10 rounded-3xl p-6 sm:p-8 transition-shadow duration-500 shadow-sm group cursor-pointer ${feature.bgClass} hover:shadow-[0_0_40px_rgba(255,255,255,0.12)] h-full w-full box-border`}
      >
        ...
        <div ref={contentRef} className="relative z-10 pointer-events-none w-full box-border">
          ...
          <h3 className="text-lg sm:text-xl font-bold text-white mb-2.5 sm:mb-3 tracking-tight">{feature.title}</h3>
          <p className="text-xs sm:text-sm text-white/80 leading-relaxed font-medium">{feature.description}</p>
        </div>
      </div>
    </div>
  );
...
const HeroSectionComponent = () => {
  ...
  return (
    ...
    >
      <div className="absolute top-[-10%] left-[-10%] w-[500px] h-[500px] bg-sky-200/40 dark:bg-blue-600/10 rounded-full blur-[120px] pointer-events-none -z-10" />
      <div className="absolute top-[20%] right-[-10%] w-[500px] h-[500px] bg-fuchsia-200/30 dark:bg-purple-600/10 rounded-full blur-[120px] pointer-events-none -z-10" />
        ...
        >
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4 select-none">
            <Link to="/stories" className="w-full sm:w-auto">
              <button className="w-full sm:w-auto px-6 sm:px-8 py-3 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-xs sm:text-sm font-bold shadow-md hover:scale-[1.02] active:scale-[0.98] transition-all duration-150 flex items-center justify-center gap-2.5 uppercase tracking-wider">
                Get Started
              </button>
            </Link>
            <Link to="/collab" className="w-full sm:w-auto">
              <button className="w-full sm:w-auto px-6 sm:px-8 py-3 rounded-xl bg-white/80 dark:bg-slate-900/40 border border-slate-200 dark:border-white/10 text-slate-700 dark:text-white text-xs sm:text-sm font-bold hover:scale-[1.02] active:scale-[0.98] transition-all duration-150 flex items-center justify-center gap-2.5 uppercase tracking-wider">
                Collab Mode
              </button>
            </Link>
          </div>
          ...
          <div className="absolute inset-0 -z-10 pointer-events-none overflow-hidden select-none">
            {stars.map((star) => (
              <span
                key={star.id}
                style={{ position: "absolute", left: star.x, top: star.y, width: star.size, height: star.size, borderRadius: "9999px", background: "white", opacity: 0.8 }}
              />
            ))}
          </div>
        </motion.div>
