  ...
  return (
    ...
    >
      ...
      {!categories || categories.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 px-4 sm:px-0 w-full box-border">
          {categories.map((category, index) => {
            // Safe fallback logic for the DOM tracking key
            const uniqueKey = category.id || category.sectionId || `cat-index-${index}`;
            return (
              <HelpCategoryCard 
                key={uniqueKey} 
                category={category} 
