...
export default function StorySceneDurationEstimator({
  ...
  const report = useMemo(() => estimateSceneDurations(story), [story]);
    ...
    <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
      ...
      <div className="space-y-4">
        {report.scenes.map((scene) => (
          <div key={scene.id} className="rounded-lg border border-zinc-700 p-5">
            ...
            <p className="mt-3 text-gray-300">Word Count: {scene.wordCount}</p>
          </div>
        ))}
      </div>
    </div>
