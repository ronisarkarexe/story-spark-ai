...
const GENRE_COLORS: Record<
  string,
  { spine: string; glow: string; text: string }
> = {
...
export default function BookShelf({ stories }: Props) {
  ...
  for (
    let i = 0;
    i < Math.max(filtered.length, BOOKS_PER_SHELF);
    i += BOOKS_PER_SHELF
  ) {
  ...
  // Stats
  const genreCounts = stories.reduce(
    (acc, s) => {
        ...
        {stories.length === 0 && (
          <div className="text-center py-20 text-white/30">
            ...
            <p className="text-sm mt-2">
              Generate some stories to fill your shelf!
            </p>
          </div>
        )}
          ...
          {shelves.map((shelfBooks, shelfIndex) => (
            <div key={shelfIndex}>
              ...
              <div className="flex items-end gap-3 px-4 pb-0">
                ...
                {/* Empty slots */}
                {Array.from({
                  length: BOOKS_PER_SHELF - shelfBooks.length,
                }).map((_, i) => (
                  <div
                    key={`empty-${i}`}
                    className="flex-1"
              ...
              <div
                style={{
                  height: "16px",
                  background:
                    "linear-gradient(180deg, #5c3d1e 0%, #3d2a15 50%, #2a1d0f 100%)",
                  borderRadius: "2px",
