...
const WriterApplicationComponent = () => {
  const { data, isLoading, refetch } =
    useGetAllWriterApplicationsQuery(undefined);
  ...
  const pendingApps: WriterApplication[] =
    data?.data?.filter((app: WriterApplication) => app.status === "pending") ||
    [];

  const processedApps: WriterApplication[] =
    data?.data?.filter((app: WriterApplication) => app.status !== "pending") ||
    [];
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-600 dark:text-slate-400">
          Review and manage user requests to become writers.
        </p>
      </div>
      ...
      <div className="bg-slate-50 border border-slate-200 dark:bg-white/[0.02] dark:border-white/[0.06] rounded-xl overflow-hidden shadow-lg">
        ...
        {pendingApps.length === 0 ? (
          ...
          <div className="divide-y divide-slate-200 dark:divide-white/[0.06]">
            {pendingApps.map((app: WriterApplication) => (
              <div
                key={app._id}
                className="p-6 transition hover:bg-slate-100/50 dark:hover:bg-white/[0.02] flex flex-col md:flex-row gap-6"
              >
                  ...
                  <div className="flex items-center gap-3 mb-2">
                    <img
                      src={
                        ...
                        `https://ui-avatars.com/api/?name=${app.user?.name}&background=random`
                      }
                      alt={app.user?.name}
                ...
                <div className="flex md:flex-col gap-3 justify-end items-end min-w-[140px]">
                  ...
                  >
                    {isUpdating && processingId === app._id
                      ? "Processing..."
                      : "Approve"}
                  </button>
                  <button
        ...
        {processedApps.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-slate-500 dark:text-slate-400">
              ...
              <tbody>
                {processedApps.map((app: WriterApplication) => (
                  <tr
                    key={app._id}
                    className="border-b border-slate-200 dark:border-white/[0.06] bg-transparent hover:bg-slate-100/50 dark:hover:bg-white/[0.02]"
                  >
