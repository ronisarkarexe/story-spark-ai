...
const SignUpComponent = () => {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      const message = (error as { data?: Array<{ message?: string }> })?.data?.[0]?.message || "Failed to resend OTP.";
      toast.error(message);
    } finally {
      setIsBusy(false);
    }
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 px-4 py-8 sm:py-12 relative overflow-x-hidden text-slate-900 dark:text-slate-100">
      <div className="absolute top-[-10%] left-[-10%] w-96 h-96 bg-blue-600/20 rounded-full blur-[120px] pointer-events-none" />
      <div className="absolute bottom-[-10%] right-[-10%] w-96 h-96 bg-indigo-600/20 rounded-full blur-[120px] pointer-events-none" />

      <div className="flex w-full max-w-md flex-col justify-center py-6 relative z-10 mx-auto">
        <div className="mb-6 text-center">
          <h2 className="text-3xl sm:text-4xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-indigo-400">
            STORY SPARK AI
          </h2>
        </div>
        ...
        <div className="bg-white dark:bg-slate-800/60 backdrop-blur-xl border border-slate-200 dark:border-slate-700/50 rounded-2xl p-5 sm:p-8 shadow-2xl w-full overflow-hidden">
          ...
          {showOtpField && registerInfo && (
            <p className="mt-2 mb-4 text-center text-xs sm:text-sm text-slate-400 px-1">
              ...
              <button type="button" onClick={handleGoBack} className="font-semibold text-blue-400 hover:text-blue-300 underline cursor-pointer">
                Change email
              </button>
            </p>
          )}
          ...
          {!showOtpField && (
            <p className="mt-2 mb-6 text-center text-xs sm:text-sm text-slate-500 dark:text-slate-400">
          ...
          {!showOtpField ? (
            <form className="flex flex-col w-full gap-5" onSubmit={handleSubmit(onSubmit)}>
              <SSInput
                ...
                icon="fi fi-rr-user" register={register} autoComplete="name"
                validation={{
                  required: "Name is required",
                  minLength: { value: 2, message: "Name must be at least 2 characters" },
                  pattern: { value: /^[A-Za-z0-9\s._]+$/, message: "Only letters, numbers, spaces, underscores, and dots are allowed" },
                }}
                error={errors.name}
              />
              <SSInput
                label="Email address" name="email" type="email" placeholder="Enter your email"
                required={true} icon="fi fi-rr-envelope" register={register} autoComplete="email"
                error={errors.email}
              />
              <SSInput
                label="Password" name="password" type="password" placeholder="Enter your password"
                required={true} icon="fi fi-rr-lock" register={register} autoComplete="new-password"
              ...
              {password.length > 0 && (
                <div className="space-y-3 w-full overflow-hidden">
                  <div className="w-full h-1.5 bg-slate-200 dark:bg-slate-700/50 rounded-full overflow-hidden"
                    role="progressbar" aria-valuenow={passedChecks} aria-valuemin={0} aria-valuemax={5}>
                    <div className={`h-full transition-all duration-300 ${barColor} ${barWidth}`} />
                  </div>
                  <p className={`text-xs font-bold uppercase tracking-wider ${textColor}`}>{strengthLabel} Password</p>
              ...
              <SSInput
                ...
                placeholder="Confirm your password" required={!showOtpField}
                icon="fi fi-rr-lock" register={register} autoComplete="new-password"
                validation={{
                  validate: (value) => {
            ...
            <div className="grid grid-cols-1 gap-5 w-full">
              <SSInput
                label="OTP" name="otp" placeholder="Enter your OTP" required={true}
                icon="fi fi-rr-key" register={register}
                validation={{
              ...
              <div className="text-center pt-1 flex flex-col items-center gap-3">
                <button type="button" onClick={handleResendOtp} disabled={cooldown > 0 || isBusy}
                  className="text-xs font-bold uppercase tracking-wider text-blue-400 hover:text-blue-300 disabled:text-slate-600 transition-colors disabled:cursor-not-allowed cursor-pointer">
                  {cooldown > 0 ? `Resend OTP (${cooldown}s)` : "Resend OTP"}
                </button>
                <button type="button" onClick={handleGoBack} disabled={isBusy}
                  className="text-xs font-bold uppercase tracking-wider text-slate-400 hover:text-slate-300 transition-colors cursor-pointer mt-1">
                  Change Email
                </button>
              </div>
            )}
            ...
            <div className="w-full">
              <div className="relative my-6 w-full">
                ...
                <div className="relative flex justify-center text-xs uppercase">
                  <span className="bg-white dark:bg-slate-800 px-4 text-slate-400 font-medium rounded-md">Or</span>
                </div>
              </div>
