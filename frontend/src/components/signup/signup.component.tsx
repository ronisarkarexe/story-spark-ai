...
const SignUpComponent = () => {
  ...
  const handleOtpValidation = async () => {
    ...
    try {
      ...
      const message =
        e?.data?.[0]?.message || e?.message || "OTP verification failed.";
      toast.error(message);
    } finally {
      setIsBusy(false);
    }
    ...
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 px-4 py-8 sm:py-12 relative overflow-x-hidden text-slate-900 dark:text-slate-100 box-border">
      ...
      <div className="flex w-full max-w-md flex-col justify-center py-6 relative z-10 px-2 sm:px-0 min-w-0 box-border mx-auto overflow-x-hidden max-w-full">
        ...
        <div className="bg-white dark:bg-slate-800/60 backdrop-blur-xl border border-slate-200 dark:border-slate-700/50 rounded-2xl p-5 sm:p-8 shadow-2xl w-full min-w-0 box-border">
          ...
          {!showOtpField ? (
            <form
              className="flex flex-col w-full min-w-0 gap-5 box-border"
                  ...
                  <ul className="space-y-1.5 list-none p-0 m-0 w-full box-border text-[11px] font-medium">
                    {PASSWORD_REQUIREMENTS.map(({ key, label }) => {
                      ...
                      return (
                        <li
                          key={key}
                          className={`flex items-center gap-1.5 ${met ? "text-green-500" : "text-slate-400"}`}
                        >
                          <span>{met ? "✅" : "❌"}</span>
                          <span>{label}</span>
                        </li>
