...
const SignUpComponent = () => {
    ...
    if (data) {
      ...
      try {
        ...
        const message =
          err?.data?.[0]?.message ||
          err?.message ||
          "Failed to send OTP. Check backend email credentials.";
        toast.error(message);
      } finally {
        setIsBusy(false);
      }
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      const err = error as { data?: Array<{ message?: string }>; message?: string };
      toast.error(err?.data?.[0]?.message || err?.message || "Failed to resend OTP");
    } finally {
      setIsBusy(false);
    }
  };
    ...
    <div className="min-h-screen w-full flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-950 px-4 py-12 relative overflow-hidden text-slate-900 dark:text-slate-100">
      ...
      <div className="flex w-full max-w-md lg:max-w-5xl flex-col lg:flex-row items-center justify-center gap-10 lg:gap-16 py-12 relative z-10 px-4">
        ...
        <div className="w-full max-w-md bg-slate-50 dark:bg-slate-800/60 backdrop-blur-xl border border-slate-200 dark:border-slate-700/50 rounded-2xl p-8 sm:p-10 shadow-2xl w-full min-w-0 overflow-hidden">
          ...
          <h3 className="text-center text-2xl font-bold tracking-tight text-slate-850 dark:text-slate-200">
          ...
          {!showOtpField && (
            <div className="relative mb-6 w-full box-border">
              ...
              <div className="relative flex justify-center text-xs">
                <span className="px-4 bg-slate-50 dark:bg-slate-800/60 text-slate-800 dark:text-slate-400 font-semibold tracking-wide rounded-md">
                  SIGN UP WITH EMAIL
                </span>
              </div>
            </div>
            ...
            <div className="w-full min-w-0 box-border">
              <div className="relative my-6 w-full box-border">
                ...
                <div className="relative flex justify-center text-xs uppercase">
                  <span className="bg-slate-50 dark:bg-slate-800 px-4 text-slate-500 dark:text-slate-400 font-medium rounded-md">
                    Or
                  </span>
                </div>
              </div>
