...
const SignUpComponent = () => {
  ...
  const handleResendOtp = async () => {
    if (!registerInfo) {
      toast.error("No registration info found. Please start over.");
      return;
    }
    setIsBusy(true);
    try {
      ...
      const err = error as { data?: Array<{ message?: string }> };
      toast.error(err?.data?.[0]?.message || "Failed to resend OTP. Please try again.");
    } finally {
      setIsBusy(false);
    }
  };
  ...
  const handleGoogleLoginSuccess = async (credentialResponse: CredentialResponse) => {
    if (credentialResponse.credential) {
      ...
      try {
        ...
        const err = error as { data?: Array<{ message?: string }> };
        toast.error(err?.data?.[0]?.message || "Google authentication failed.");
      } finally {
        setIsBusy(false);
      }
    }
