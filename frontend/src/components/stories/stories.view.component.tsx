...
const StoriesViewComponent: React.FC<StoriesComponentProps> = ({
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      // 4.5. User Prompt Section
      const promptToUse = selectedStory.prompt || selectedStory.enhancedPrompt;
      if (promptToUse) {
        doc.setFont("helvetica", "bold");
        doc.setFontSize(10);
        doc.setTextColor(99, 102, 241); // Brand Indigo
        doc.text("Prompt:", leftMargin, yCursor);
        yCursor += 5;
        ...
        const splitPrompt = doc.splitTextToSize(`"${promptToUse}"`, printableWidth);
...
`;
  ...
  const handleExportTXT = () => {
    ...
    try {
      ...
      const content = selectedStory.content || "";
      const tag = selectedStory.tag || "General";
      const language = selectedStory.language || "English";
      const promptToUse = selectedStory.prompt || selectedStory.enhancedPrompt || "N/A";
      const formattedDate = new Date().toLocaleString();

      const txtContent = `==================================================
STORY GENERATED BY STORYSPARK AI
==================================================
Title: ${title}
Date: ${formattedDate}
Genre: ${tag}
...
`;
      ...
      const fileName = title.toLowerCase().replace(/[^a-z0-9]/gi, "_").toLowerCase() || "story";
      ...
      toast.success("TXT Story downloaded!");
    } catch (error) {
      console.error(error);
