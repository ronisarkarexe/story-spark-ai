...
const StoriesViewComponent: React.FC<StoriesComponentProps> = ({
  ...
  const handleExport = async (format: "pdf" | "epub") => {
    ...
    try {
      ...
      toast.loading("Compiling e-book resources...", { id: toastId });

      if (format === "pdf") {
        await exportStoryToPDF(selectedStory.title, selectedStory.content, base64Image);
      } else {
        await exportStoryToEPUB(selectedStory.title, selectedStory.content, base64Image, selectedStory.tag);
      }
      ...
      toast.error(`Failed to export ${format.toUpperCase()}. Please try again.`, { id: toastId });
    } finally {
  ...
  // Sync selectedStory when stories array changes
  useEffect(() => {
    if (stories && stories.length > 0) {
      if (!selectedStory || !stories.some((s) => s.uuid === selectedStory.uuid)) {
  ...
  // Cache original story contents
  useEffect(() => {
    if (stories) {
      stories.forEach((story) => {
        if (!originalStoryContent[story.uuid]) {
          setOriginalStoryContent((prev) => ({
            ...prev,
            [story.uuid]: story.content,
          }));
        }
      });
    }
  }, [stories, originalStoryContent]);
  ...
  return (
    <div ref={storyScrollContainerRef} className="w-full min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-300 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto pt-8 pb-16 relative overflow-hidden box-border">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-100 dark:border-white/5 select-none relative z-10">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="relative inline-block text-left" ref={dropdownMenuRef}>
                  ...
                  >
                    {exportState === "idle" ? "📥 Export" : exportState === "processing" || exportState === "compiling" ? "Compiling..." : "Export"} <i className="fa-solid fa-chevron-down text-[10px]" />
                  </button>
            ...
            {isGeneratingEndings ? (
                  ...
                  return (
                    <div className="bg-slate-50 dark:bg-slate-900/40 rounded-xl p-6 border border-slate-200 dark:border-slate-700/30">
                      ...
                      <div className="space-y-4">
                        <div className="bg-white dark:bg-slate-950/40 p-4 rounded-xl border border-slate-200 dark:border-slate-700/30 leading-relaxed text-slate-600 dark:text-slate-300 text-sm italic shadow-inner whitespace-pre-wrap">
                          "{currentEndingData.ending}"
                        </div>
                        <details className="group border border-slate-200 dark:border-slate-700/30 rounded-xl overflow-hidden bg-white dark:bg-transparent">
                          <summary className="list-none flex items-center justify-between p-4 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider hover:text-slate-800 dark:hover:text-slate-200 cursor-pointer select-none">
                            <span>Preview Full Story with this Ending</span>
                            <span className="transition-transform duration-150 group-open:rotate-180 text-xs">▼</span>
                          </summary>
                          <div className="p-4 border-t border-slate-200 dark:border-slate-700/30 text-sm text-slate-600 dark:text-slate-400 leading-relaxed max-h-60 overflow-y-auto whitespace-pre-wrap">
              ...
              <div className="flex flex-col items-center justify-center py-8 bg-slate-100 dark:bg-slate-900/20 border border-dashed border-slate-300 dark:border-slate-700/40 rounded-xl">
                <button
                  ...
                  className="rounded-xl px-6 py-3 bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 text-white font-bold transition-all duration-300 transform hover:scale-105 hover:shadow-lg hover:shadow-purple-500/30 flex items-center gap-2 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                >
