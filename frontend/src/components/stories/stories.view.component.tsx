                ...
                {sentenceSegments.length > 0 ? (
                  sentenceSegments.map((segment: StorySentenceSegment) => {
                    ...
                    return (
                      <span
                        key={segment.id}
                        className={isActiveSentence ? "text-slate-100 font-medium transition-colors duration-300 rounded-md bg-indigo-500/20 px-0.5 py-0.5 text-indigo-800 dark:text-indigo-100 ring-1 ring-indigo-400/30" : undefined}
                      >
                  ...
                  <span dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(selectedStory.content) }} />
                )}
              </p>
            </div>
