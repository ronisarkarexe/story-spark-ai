...
import { getShortenedText, ITopicData, topicsData, getWordCount, SELECTED_TOPIC_CLASSES, CharacterProfile } from "./stories.utils";
...
const [, setShowRemix] = useState<boolean>(false);
  ...
  const handleContinueReading = () => {
    const element = storyContentRef.current;
    if (element) {
      const saved = localStorage.getItem(`story-progress-${selectedStory?.uuid}`);
      if (saved) {
        const percentage = Number(saved) / 100;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-fade-in-up">
        <div className="col-span-1 lg:col-span-8 flex flex-col">
          ...
          <div className="bg-slate-800/80 backdrop-blur-xl border border-slate-700/50 p-8 rounded-2xl shadow-2xl relative overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
              ...
              <div className="flex flex-wrap items-center gap-2 relative z-10">
                ...
                <button
                  type="button"
                  className="rounded-lg px-4 py-2 bg-indigo-700 text-white font-semibold hover:bg-indigo-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            ...
            <div id="story-content" ref={storyContentRef} className="prose prose-invert max-w-none text-slate-300 leading-relaxed tracking-wide relative z-10 max-h-[600px] overflow-y-auto pr-4">
          ...
          {characterProfiles.length > 0 && (
            <div className="mt-6 bg-slate-800/80 backdrop-blur-xl border border-slate-700/50 p-6 rounded-2xl shadow-2xl relative z-10">
