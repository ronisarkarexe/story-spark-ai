...
export const FloatingChatWidget: React.FC = () => {
  ...
  const handleSendMessage = async (textToSend: string) => {
    ...
    try {
      ...
      const apiError = err as { data?: unknown[] | { message?: string } };
      if (apiError?.data) {
        if (Array.isArray(apiError.data) && apiError.data.length > 0) {
          const firstError = apiError.data[0] as { message?: string };
          errMsg = firstError.message || errMsg;
        } else if (
          typeof apiError.data === "object" &&
          apiError.data !== null &&
          "message" in apiError.data
        ) {
          errMsg = String((apiError.data as { message?: string }).message);
        }
