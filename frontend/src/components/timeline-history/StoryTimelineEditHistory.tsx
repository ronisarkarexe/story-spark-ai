...
export default function StoryTimelineEditHistory({
  ...
  const history = useMemo(() => generateEditHistory(story), [story]);
        ...
        {history.map((checkpoint) => (
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-semibold text-white">{checkpoint.title}</h3>
            ...
            <p className="mt-3 text-gray-300">{checkpoint.summary}</p>

            <div className="mt-4 flex flex-wrap gap-3">
              <button
                onClick={() => setSelectedVersion(checkpoint.content)}
              ...
              <button
                onClick={() =>
                  onRestore(restoreStoryVersion(history, checkpoint.id))
                }
