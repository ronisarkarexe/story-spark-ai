...
export default function CollabRoom() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Request room info
      collabSocket.emit("collab:get_room", { roomId }, (response: { room?: Room } | undefined) => {
      ...
      const handleRoomUpdated = (data: { room?: Room } | undefined) => {
        if (data && data.room) {
          setRoom(data.room);
        }
      };

      const handleStoryUpdated = (data: { story?: StoryChunk[] } | undefined) => {
        if (data && data.story) {
      ...
      collabSocket.on("collab:error", (data: { message?: string } | undefined) => {
  ...
  const handleAddText = () => {
    ...
    if (socket) {
      (socket as any).emit("collab:add_text", {
        roomId,
        userId: user.userId,
        text: newText,
      });
  ...
  const handleAIContinue = () => {
    const socket = getSocketIo();
    if (socket) {
      (socket as any).emit("collab:ai_continue", { roomId });
    }
  };
