...
import { connectSocket } from "../../socket/socket.oi"; // Ensure this filename matches your path (.oi vs .io)
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-[#0d0d14] dark:text-white flex items-center justify-center px-4 transition-colors duration-300 w-full box-border relative overflow-hidden">
      ...
      <div className="max-w-md w-full relative z-10 box-border">
        ...
        {/* Unified Main Control Card Frame */}
        <div className="bg-white dark:bg-[#111827]/40 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-2xl sm:rounded-3xl p-6 sm:p-10 shadow-sm hover:shadow-xl transition-shadow duration-300 w-full box-border text-center">
          ...
          {error && (
            <div className="bg-red-500/5 border border-red-500/10 rounded-xl px-4 py-3 text-red-500 dark:text-red-400 text-xs font-semibold mb-6 text-left">
              {error}
            </div>
          )}
          ...
          <div className="space-y-5 w-full box-border">
            <button
              onClick={createRoom}
              disabled={isCreating}
              className="w-full h-12 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 disabled:opacity-50 text-white text-xs sm:text-sm font-bold uppercase tracking-wider rounded-xl shadow-md shadow-blue-500/10 transition-all duration-150 active:scale-[0.98] select-none cursor-pointer border-none outline-none"
            >
              {isCreating ? "Creating Room..." : "✨ Create a New Story Room"}
            </button>
          </div>

          {/* Form Context Inline Section Splitter */}
          ...
          {/* Join Existing Workspace Forms Inputs */}
          <div className="flex gap-2 w-full box-border">
            ...
            <button
              onClick={joinRoom}
              className="h-11 px-5 rounded-xl bg-slate-900 text-white dark:bg-white dark:text-slate-900 text-xs font-bold uppercase tracking-wider hover:bg-slate-800 dark:hover:bg-slate-100 transition-colors active:scale-[0.98] cursor-pointer shadow-sm shrink-0 border-none outline-none"
            >
              Join 🚀
            </button>
          </div>

          {/* Bottom Features Bullet Grid Grid View */}
