...
export default function CollabHome() {
  ...
  const createRoom = () => {
    ...
    try {
      ...
      const collabSocket = manager?.of?.("/collab");
      if (!collabSocket) {
        setError(
          "Socket.IO namespace connection failed. Please check VITE_SOCKET_URL in frontend/.env"
        );
        setIsCreating(false);
        return;
      }
      ...
      collabSocket.emit(
        ...
        (response: unknown) => {
          if (
            ...
            response !== null &&
            "roomId" in response &&
            typeof (response as { roomId?: string }).roomId === "string"
          ) {
