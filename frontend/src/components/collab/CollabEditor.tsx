  ...
  useEffect(() => {
    ...
    const renderRemoteCursors = () => {
      ...
      states.forEach((state: any, clientId: number) => {
        ...
        if (user && cursor) {
          ...
          const existing = (quillCursorsRef as any).current?.cursors?.[
            cursorId
          ];
          if (!existing) {
            (quillCursorsRef as any).current?.createCursor(
              cursorId,
              user.name,
              user.color,
            );
    ...
    if (socketUrl) {
      ...
      socket.on("connect_error", (err: Error) => {
        console.warn(
          "[Story Spark] Collab editor socket connection error:",
          err.message,
        );
      });
      ...
      awareness.on("update", ({ added, updated, removed }: any) => {
        const awUpdate = awareness.encodeUpdate(
