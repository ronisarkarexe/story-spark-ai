...
const GithubcontributorsComponent: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const GithubcontributorData = async () => {
      try {
        ...
        if (!controller.signal.aborted && repoData.stargazers_count !== undefined) {
  ...
  const totalCommits = githubcontributors.reduce((acc, curr) => acc + curr.contributions, 0);

  return (
    <section
      className="relative overflow-hidden py-20 px-4 sm:px-6 lg:px-8 bg-white dark:bg-[#050816]"
      ref={contributorsRef}
    >
      {/* Background Glow */}
      ...
      <div className="relative max-w-7xl mx-auto">
        ...
        <div className="grid lg:grid-cols-2 gap-14 items-center mb-16">
          ...
          <div className="max-w-3xl">
            <h1 className="text-[56px] md:text-[72px] font-extrabold leading-[1.05] tracking-[-2px] text-gray-900 dark:text-white">
              Our GitHub{" "}
            ...
            <p className="mt-6 text-[30px] text-gray-700 dark:text-gray-300 font-medium">
              Powered by amazing contributors 💻
            </p>

            <div className="mt-12 space-y-3">
              <p className="text-[18px] md:text-[20px] text-gray-600 dark:text-gray-400 leading-relaxed">
          ...
          <div className="hidden lg:flex justify-center items-center relative">
            ...
            <img
              src={githubHero}
              alt="GitHub Hero"
              className="w-full max-w-[620px] object-contain drop-shadow-[0_0_60px_rgba(139,92,246,0.45)] animate-[float_5s_ease-in-out_infinite]"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 xl:grid-cols-4 gap-6 mb-24">
          {[
            ...
            <div
              key={index}
              className="h-[118px] rounded-[28px] border border-gray-200 dark:border-white/10 bg-white dark:bg-[#0b1023]/90 shadow-[0_4px_30px_rgba(0,0,0,0.06)] dark:shadow-none backdrop-blur-xl px-6 flex items-center justify-between overflow-hidden"
            >
              {/* LEFT */}
              ...
              {/* RIGHT ICON */}
              <i
                className={`fa-solid fa-chart-line text-4xl opacity-60 ${
                  item.color === "violet"
                    ? "text-violet-400"
                    : item.color === "yellow"
                    ? "text-yellow-400"
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-10">
          ...
          <button
            ...
            className="hidden md:flex items-center gap-4 text-violet-400 hover:text-violet-300 text-2xl font-semibold transition-colors cursor-pointer"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {DisplayedContributors.map((contributor, index) => {
            const rankStyles = [
              {
                border: "border-yellow-500/40 shadow-[0_0_40px_rgba(250,204,21,0.18)]",
                badge: "bg-yellow-500/10 border-yellow-500/30 text-yellow-400",
              },
              {
                border: "border-gray-300/40 shadow-[0_0_40px_rgba(255,255,255,0.12)]",
                badge: "bg-gray-400/10 border-gray-400/30 text-gray-300",
              },
              {
                border: "border-orange-500/40 shadow-[0_0_40px_rgba(251,146,60,0.16)]",
            ...
            return (
              <div
                key={contributor.id}
                className={`group relative isolate overflow-hidden rounded-[26px] border ${style.border} bg-white dark:bg-[#0b1023]/95 shadow-[0_4px_30px_rgba(0,0,0,0.06)] dark:shadow-none backdrop-blur-2xl w-full mx-auto px-5 py-5 hover:-translate-y-3 transition-all duration-500`}
              >
                ...
                {/* Secondary Glow */}
                <div
                  className={`absolute bottom-0 right-0 w-[220px] h-[220px] blur-[90px] rounded-full opacity-30 ${
                    index === 0
                      ? "bg-yellow-500"
                      : index === 1
                      ? "bg-blue-400"
                ...
                <div className="text-center">
                  ...
                  {/* Commit Badge */}
                  <div
                    className={`mt-6 inline-flex items-center gap-3 px-4 py-2 rounded-2xl border ${style.badge} text-sm`}
                  >
        ...
        <div className="flex md:hidden justify-center mt-10">
          <button
            ...
            className="px-7 py-3 rounded-full bg-gradient-to-r from-violet-600 to-blue-600 text-white font-semibold cursor-pointer"
          >
            {showAll ? "Show Less" : "Show More"}
          </button>
        </div>
        ...
        {/* CTA */}
        <div className="mt-20 rounded-[36px] border border-violet-500/20 bg-white dark:bg-white/[0.03] backdrop-blur-2xl p-10 flex flex-col lg:flex-row items-center justify-between gap-5">
          <div>
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="px-8 py-5 rounded-2xl bg-gradient-to-r from-violet-600 to-fuchsia-600 hover:scale-105 text-white font-bold text-lg shadow-2xl transition-all cursor-pointer"
          >
            Contribute Now →
          </a>
        </div>
