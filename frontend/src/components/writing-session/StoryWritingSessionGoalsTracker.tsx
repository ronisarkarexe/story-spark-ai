...
export default function StoryWritingSessionGoalsTracker({ story }: Props) {
  ...
  const progress = useMemo(
    () => calculateSessionProgress(story, goals, startTime),
    [story, goals],
  );
  ...
  return (
    <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-6">
      ...
      <div className="space-y-5">
        <ProgressBar label="Word Goal" value={progress.wordProgress} />

        <ProgressBar label="Writing Time" value={progress.timeProgress} />

        <ProgressBar label="Chapter Goal" value={progress.chapterProgress} />
      </div>
