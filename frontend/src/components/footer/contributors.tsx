...
export default function ContributorsComponent() {
    ...
    const fetchContributors = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          const sorted = data
            .filter((c: Contributor) => c.contributions >= 1)
      ...
      if (heroRef.current) {
        ...
        if (dots.length) {
          heroTl.fromTo(
            dots,
            { scale: 0, opacity: 0 },
            { scale: 1, opacity: 1, duration: 0.6, ease: "elastic.out(1, 0.4)", stagger: 0.08 },
            "-=0.3"
          );
        }
      }
  ...
  return (
    <div
      ...
      style={{
        background: "linear-gradient(180deg, #030712 0%, #0c0a1f 35%, #0f172a 65%, #030712 100%)",
      }}
    >
      <ParticleField />

      {/* Ambient Glow Orbs */}
      <div className="absolute top-0 left-1/4 w-[500px] h-[500px] rounded-full pointer-events-none bg-indigo-600/5 blur-[120px]" />
      <div className="absolute top-1/3 right-0 w-[400px] h-[400px] rounded-full pointer-events-none bg-purple-600/5 blur-[100px]" />
      <div className="absolute bottom-0 left-0 w-[350px] h-[350px] rounded-full pointer-events-none bg-blue-600/5 blur-[100px]" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-20 md:py-28">
        ...
        <div ref={heroRef} className="text-center mb-20 md:mb-28">
          <div className="hero-badge inline-flex items-center gap-2.5 rounded-full border border-indigo-500/20 bg-indigo-500/5 px-5 py-2 text-sm text-indigo-300 mb-8">
            ...
            <Sparkles size={14} className="inline" /> Open Source Community
          </div>

          <div style={{ perspective: "600px" }}>
            <h1 className="hero-title-line text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-black tracking-tight leading-none text-white/90">
              Meet Our
            </h1>
            <h1 className="hero-title-line text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-black tracking-tight leading-none mt-3">
              <span
                className="bg-clip-text text-transparent bg-gradient-to-r from-blue-400 via-indigo-300 to-purple-400"
                style={{
        ...
        >
          {[
            ...
            <div
              key={i}
              className="stat-card relative rounded-2xl p-6 overflow-hidden cursor-default"
              style={{
                background: "linear-gradient(135deg, rgba(15,23,42,0.7), rgba(30,27,75,0.4))",
            ...
            >
              ...
              <div className="relative z-10">
                <div className={`w-11 h-11 rounded-xl ${stat.iconBg} flex items-center justify-center mb-4 ${stat.iconColor}`}>
                  {stat.icon}
                </div>
        ...
        <div className="flex items-center gap-4 mb-12">
          ...
          <h2 className="text-2xl md:text-3xl font-bold text-white flex items-center gap-3 select-none">
