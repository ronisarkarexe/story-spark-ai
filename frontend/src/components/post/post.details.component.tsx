...
const PostDetailsComponent = () => {
  ...
  const [selectedVersionForBranch, setSelectedVersionForBranch] = useState<
    string | null
  >(null);
  ...
  const { data: storyTree } = useGetStoryTreeQuery(id || "", {
    skip: !id || !showTree,
  });
        ...
        <div className="rounded-lg shadow-sm bg-gray-50 border border-gray-200 text-slate-900 mb-10 dark:bg-[#0f172a]/70 dark:border-transparent dark:text-white">
          <div className="p-8">
            ...
            {isEditing ? (
              ...
              <>
                <h1
                  className={`text-4xl font-bold text-slate-900 dark:text-gray-300 leading-tight ${post?.language ? "mb-2" : "mb-4"}`}
                >
                  {post?.title}
                </h1>
            ...
            <div className="mt-12">
              ...
              <SimilarStories stories={relatedPost || []} />
            </div>
          </div>
        </div>
      </div>
