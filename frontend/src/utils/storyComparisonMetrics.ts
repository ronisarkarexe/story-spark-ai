...
export function calculateStoryMetrics(story: string): StoryMetrics {
  ...
  return {
    ...
    dialoguePercentage: Math.min(
      100,
      Math.round(((story.match(/"/g)?.length || 0) / 2) * 5),
    ),
    pacing: wordCount > 800 ? 90 : wordCount > 400 ? 75 : 60,
    sentiment: "Neutral",
  };
}

export function compareStories(storyA: string, storyB: string) {
  return {
