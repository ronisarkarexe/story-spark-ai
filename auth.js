...
const GOOGLE_CLIENT_ID =
  typeof window !== "undefined" && window.VITE_GOOGLE_CLIENT_ID
...
function setAlert(variant, message) {
  ...
  const iconClass =
    variant === "success"
      ? "fi fi-rr-check-circle text-green-400"
      : variant === "error"
        ? "fi fi-rr-cross-circle text-rose-400"
...
function validatePassword(showInline) {
  ...
  else if (!/\d/.test(value)) message = "Include at least one number.";
...
function setSubmitting(submitting) {
  ...
  const spinner =
    document.getElementById("submit-btn-spinner") ||
...
function initParticleSystem() {
  ...
  class Particle {
    ...
    update(t) {
      ...
      this.y += this.vy;
      this.alpha =
        this.baseAlpha + Math.sin(t * this.pulseSpeed + this.pulsePhase) * 0.2;
...
function toggleAuthMode(mode) {
  ...
  setTimeout(() => {
    ...
    const submitBtnText =
      document.getElementById("btn-label") ||
      document.getElementById("submit-btn-text"); // Support both IDs
    ...
    if (mode === "signup") {
      ...
      if (tabSignup)
        tabSignup.className =
          "auth-tab-button flex-1 pb-3 font-label-caps text-label-caps text-primary border-b-2 border-primary transition-all duration-300 active";
      if (tabSignin)
        tabSignin.className =
          "auth-tab-button flex-1 pb-3 font-label-caps text-label-caps text-on-surface-variant border-b-2 border-transparent hover:text-on-surface transition-all duration-300";
...
async function handleFormSubmit(e) {
  ...
  const okConfirmPassword =
    currentMode === "signup" ? validateConfirmPassword(true) : true;

  const isValid =
    currentMode === "signup"
      ? okName && okEmail && okPassword && okConfirmPassword
      : okEmail && okPassword;
  if (!isValid) {
  ...
  const password =
    passwordField && passwordField.value ? passwordField.value : "";
  const rememberMe = rememberCheckbox ? rememberCheckbox.checked : false;

  setSubmitting(true);
  setAlert(
    "info",
    currentMode === "signup" ? "Creating your account…" : "Signing you in…",
  );

  try {
    const endpoint =
      currentMode === "signup" ? "/api/v1/auth/register" : "/api/v1/auth/login";
    const body =
      currentMode === "signup"
        ? { email, name, password }
    ...
    if (currentMode === "signin") {
      setAlert(
        "success",
        `Signed in as <span class="font-semibold">${escapeHtml(email)}</span>. Redirecting…`,
      );
      setTimeout(() => {
      ...
      const greeting = name
        ? `Welcome, <span class="font-semibold">${escapeHtml(name)}</span>!`
        : "Welcome!";
      setAlert(
        "success",
        `${greeting} Your account is ready. Redirecting to sign in…`,
      );
...
function initGoogleAuth() {
  ...
  if (GOOGLE_CLIENT_ID === "YOUR_GOOGLE_CLIENT_ID.apps.googleusercontent.com") {
    console.warn(
      "Google Sign-In is not configured (VITE_GOOGLE_CLIENT_ID missing) — skipping initialization.",
    );
    return;
  }
...
async function handleGoogleCredentialResponse(response) {
  try {
    ...
    if (!token) {
      setAlert(
        "error",
        "Google login failed. Invalid token received from server.",
      );
      return;
    }
...
function handleGoogleSignIn() {
  ...
  if (typeof google === "undefined" || !google.accounts) {
    alert(
      "Google Sign-In library is still loading. Please try again in a moment.",
    );
    return;
  }
