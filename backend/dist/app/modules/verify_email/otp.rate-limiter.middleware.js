...
const cleanupInterval = setInterval(() => {
    ...
    for (const key of Object.keys(rateLimitStore)) {
        ...
        if ((now > record.blockUntil && record.attempts >= PHASE_2_MAX_ATTEMPTS) ||
...
const otpRateLimiter = (req, res, next) => {
    ...
    if (record.blockUntil > now) {
        ...
        if (record.attempts >= PHASE_2_MAX_ATTEMPTS) {
            throw new api_error_1.default(http_status_1.default.TOO_MANY_REQUESTS, `You have been blocked from verifying due to too many attempts. Please try again after ${hoursLeft} hours.`);
        }
        else {
            throw new api_error_1.default(http_status_1.default.TOO_MANY_REQUESTS, `Too many OTP verification attempts. Please try again after ${minsLeft} minutes.`);
        }
    }
