...
const getPersonalizedRecommendations = async (token: ITokenPayload) => {
  ...
  if (readingPreferences) {
    const favoriteGenres = (readingPreferences.favoriteGenres ? [...readingPreferences.favoriteGenres] : [])
      .sort((a, b) => b.count - a.count)
      .slice(0, 3)
      .map(g => g.name);
      
    const favoriteEmotions = (readingPreferences.favoriteEmotions ? [...readingPreferences.favoriteEmotions] : [])
      .sort((a, b) => b.count - a.count)
      .slice(0, 3)
      .map(e => e.name);
