...
const getAllWriterApplicationUsers = async (): Promise<IUser[]> => {
  const result = await User.find({ isApplyForWriter: true }).select(
    "-password -pendingEmailToken -pendingEmailTokenExpires"
  );
  return result;
};
...
const getProfileInfo = async (token: ITokenPayload) => {
  const { email } = token;
  const user = await User.findOne({ email: email })
    .select("-password -pendingEmailToken -pendingEmailTokenExpires")
    .populate("followers", "name profile")
    .populate("following", "name profile");
  if (!user) {
    throw new ApiError(httpStatus.BAD_REQUEST, "User not found!");
