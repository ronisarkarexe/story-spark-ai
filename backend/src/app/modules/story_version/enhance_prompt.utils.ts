...
export const enhancePrompt = (prompt: string, context?: string): string => {
  const compressedContext = context ?? "No previous context";

  return `You are a creative writing assistant. Rewrite the following story prompt to be more vivid, specific, and engaging. Add a character name, setting details, and a central conflict. Return ONLY the enhanced prompt, nothing else. Do not add any explanation or prefix. Context: ${compressedContext}`;
};
