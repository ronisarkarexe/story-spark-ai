...
describe("Character Network Extraction Logic", () => {
  it("should extract characters and relationships offline correctly", () => {
    const story =
      "Merlin walked through the forest toward the castle. Elena watched from the palace window as Merlin approached.";
  ...
  it("should calculate importanceScore and relationships properly", () => {
    const story =
      "Merlin and Elena talked in the library. Merlin loved books, but Elena preferred sword fighting.";
    const result = extractCharacterNetworkOffline(story);
  ...
  it("should set higher importanceScore for character with more appearances", () => {
    const story =
      "Merlin cast a spell. Merlin opened the gate. Merlin spoke to Elena.";
    const result = extractCharacterNetworkOffline(story);
    const merlin = result.characters.find((c) => c.name === "Merlin");
  ...
  it("should not produce duplicate relationships between same pair", () => {
    const story =
      "Merlin met Elena. Elena met Merlin. Merlin and Elena talked again.";
    const result = extractCharacterNetworkOffline(story);
...
describe("Character Network Controller", () => {
  ...
  it("should retrieve character network and send successful response", async () => {
    ...
    await StoryVersionController.getCharacterNetwork(
      mockReq as Request,
      mockRes as Response,
      mockNext,
    );

    expect(StoryVersionService.getCharacterNetwork).toHaveBeenCalledWith(
      "656b856b3e34a6efc023dabc",
      "user123",
    );
