...
export function serializeLore(lore: LorePayload): string {
  ...
  if (lore.characters.length) {
    parts.push(
      "Characters: " + lore.characters.map((c) => c.name).join(", ")
    );
  }
  ...
  if (lore.core_events.length) {
    parts.push(
      "Key events: " + lore.core_events.slice(-5).join(" | ")
    );
  }
...
export function compressContext(
  ...
  const MAX =
    maxTokens ??
    parseInt(process.env.MAX_CONTEXT_TOKENS ?? "4096", 10);
  ...
  for (let i = nodes.length - 1; i >= 0; i--) {
    ...
    const nodeTokens = countTokens(node.text);

    if (nodeTokens > budget) {
      break;
    }
