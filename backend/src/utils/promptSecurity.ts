...
const FORBIDDEN_PATTERNS: RegExp[] = [
  ...
  /dan\s+mode/i,
  /developer\s+mode/i,
  /pretend\s+(you\s+are|to\s+be)\s+(?:a\s+|the\s+)?(?:different|unrestricted|unfiltered|evil|bad|another|developer|system)/i,
  /act\s+as\s+(if\s+you\s+are\s+)?(?:a\s+|the\s+)?(?:different|unrestricted|unfiltered|evil|bad|another|developer|system)/i,
  /you\s+are\s+now\s+(?:a\s+|the\s+)?(?:different|unrestricted|unfiltered|evil|bad|another|developer|system)/i,
