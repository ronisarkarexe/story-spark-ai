...
function drawGridCards(cards) {
  ...
  cards.forEach((card, index) => {
    ...
    // Top accent line
    doc.setFillColor(
      ...(card.accent === "cyan" ? COLOR_SECONDARY : COLOR_PRIMARY),
    );
    ...
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9.5);
    doc.text(card.desc, x + 20, startY + 55, {
...
doc.setFont("helvetica", "bold");
doc.setFontSize(12);
doc.text("THE AI-NATIVE CLINICAL COMPANION", pageWidth / 2, pageHeight / 2, {
  align: "center",
});
...
doc.setFontSize(10);
doc.text(
  "Built by Abdisa Jemal Hasen & Sonu Sharma (Team WEBTitans)",
  pageWidth / 2,
  pageHeight - 70,
  { align: "center" },
);
...
drawGridCards([
  {
    ...
    desc: "The platform captures live telemetry metrics like HRV (Heart Rate Variability), skin temperature, and sleep architecture via wearable integration.",
  },
  {
    title: "Background AI Operations",
    desc: "To guarantee an uninterrupted experience, heavy pattern-recognition (analyzing a 30-day longitudinal history of 34 symptoms) is offloaded to our background LLM engine.",
  },
]);
...
drawGridCards([
  {
    title: "Complexity",
    desc: "A standard 12-minute doctor visit is completely insufficient to track and diagnose up to 34 distinct, interacting symptoms spanning over decades.",
    accent: "cyan",
  },
  {
    title: "Vulnerability",
    desc: "80% of OB-GYNs lack formal menopause training. Over 1.1 Billion women are affected globally, resulting in $150B lost annually in workplace productivity due to unmanaged symptoms.",
    accent: "cyan",
  },
  {
    title: "Dependency",
    desc: "Existing FemTech platforms are merely menstrual trackers. They act as static lookup tables instead of natively analyzing complex perimenopausal transitions over time.",
    accent: "cyan",
  },
]);
...
drawGridCards([
  {
    title: "Data Privacy (Local-First)",
    desc: "Isolates PII (Personally Identifiable Information) from the AI layer to maintain strict HIPAA-aligned safety protocols and prevent data collisions.",
  },
  {
    title: "Real-Time Safety Thread",
    desc: "Shifts emergency keyword classification to a prioritized loop. Instantly bypasses the AI and surfaces crisis lifelines if severe medical symptoms are detected.",
  },
  {
    title: "Context AI Engine",
    desc: "Injects the user's 30-day continuous symptom history directly into the LLM. Analyzes daily metrics against NAMS 2022 clinical guidelines to forecast trigger events natively.",
  },
]);
...
drawGridCards([
  {
    title: "Pattern Recognition (Concurrency)",
    desc: "Handles overlapping health inputs smoothly. Uses advanced RAG to cross-reference multiple clinical databases (vasomotor, sleep, psychiatry) instantly.",
    accent: "cyan",
  },
  {
    title: "Clinical Stability",
    desc: "Keeps AI outputs strictly evidence-based. Prevents AI hallucinations and medical over-reliance by refusing to recommend specific prescription dosages.",
    accent: "cyan",
  },
  {
    title: "Emergency Persistence",
    desc: "Enforces secure escalation paths. Any input matching crisis keywords triggers a hard-coded safety override.",
    accent: "cyan",
  },
]);
...
doc.text(
  ...
  40,
  160,
  { maxWidth: 440, lineHeightFactor: 1.4 },
);
...
doc.setFont("helvetica", "normal");
doc.setFontSize(7.5);
doc.text("FORECAST ACCURACY", hudX + hudWidth / 2, hudY + 122, {
  align: "center",
});
...
doc.setFont("helvetica", "bold");
doc.setFontSize(9);
doc.text("MODEL: LUNA-TRANSITION-V1", hudX + hudWidth / 2, hudY + 200, {
  align: "center",
});
...
const stackItems = [
  {
    ...
    val: "React Native framework with high-contrast Tech-Noir custom Tailwind CSS design.",
  },
  {
    title: "Intelligence Plane",
    val: "Anthropic Claude API for intent triage and RAG-grounded response generation.",
  },
  {
    title: "Backend Concurrency",
    val: "Node.js API with PostgreSQL, running via Replit Agent for rapid live deployment.",
  },
];
...
stackItems.forEach((item, index) => {
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8);
  doc.text(item.val, col2X + 15, y + 12, {
...
const steps = [
  {
    p: "P1",
    name: "Hackathon Baseline",
    desc: "Deploying conversational AI & emergency triage safeguard.",
  },
  {
    p: "P2",
    name: "Closed Beta",
    desc: "Onboarding 50 real users to validate trigger-detection.",
  },
  {
    p: "P3",
    name: "Zenith Scalability",
    desc: "Stripe payment & B2B employer telehealth pilots.",
  },
];
...
steps.forEach((step, index) => {
  ...
  doc.setFont("helvetica", "normal");
  doc.setFontSize(7.5);
  doc.text(step.desc, col3X + 15, y + 14, {
    maxWidth: colWidth - 30,
    lineHeightFactor: 1.3,
  });
});
