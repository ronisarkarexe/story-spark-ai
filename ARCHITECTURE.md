...
The current repository uses the env names shown below in `backend/.env.example` and `frontend/.env.example`, so those files should be treated as the source of truth when wiring local setup or deployment.
...
Story cover artwork is fetched dynamically from Unsplash at runtime, so there is no local static asset directory to copy for the cover-image flow.
